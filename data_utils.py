...
from mecab_utils import MeCabTokenizer # ★追加
...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def get_audio_text_speaker_pair(self, audiopath_sid_text):
        ...
        # ★追加: テキストからMeCab IDを取得
        mecab_ids = self.mecab_tokenizer.text_to_ids(text)

        spec, wav = self.get_audio(audiopath)
        sid = torch.LongTensor([int(self.spk_map[sid])])
        style_vec = torch.FloatTensor(np.load(f"{audiopath}.npy"))
        if self.use_jp_extra:
            return (phones, spec, wav, sid, tone, language, ja_bert, style_vec, mecab_ids)
        else:
            return (
                phones,
                spec,
...
class TextAudioSpeakerCollate:
    ...
    def __call__(self, batch):
        ...
        # ★追加: mecab_ids の最大長を計算 (デバッグ用や整合性チェック用)
        # max_mecab_len = max([len(x[-1]) for x in batch])
        ...
        wav_padded.zero_()
        bert_padded.zero_()
        # ★追加: mecab_ids (バッチの最後の要素) をリストとして取り出す
        mecab_ids_list = [x[-1] for x in batch]
        # ★追加: パディングしてTensor化
        mecab_ids_padded = self.mecab_tokenizer.batch_text_to_ids_from_ids(mecab_ids_list)
        if not self.use_jp_extra:
        ...
        for i in range(len(ids_sorted_decreasing)):
            ...
            if self.use_jp_extra:
                ...
                style_vec[i, :] = row[9]
        # 並び替え (ids_sorted_decreasing に合わせる)
        mecab_ids_padded = mecab_ids_padded[ids_sorted_decreasing]
