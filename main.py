...
try:
    ...
    print("[ERROR] 必要なモジュール(transcribe_func, answer_generator, text_to_speech)が見つかりません。")
    exit(1)
...
print(f"[INFO] '{PROCESSING_DIR}' ディレクトリを /download としてマウントしました。")
...
async def process_audio_file(audio_path: str, original_filename: str, websocket: WebSocket):
    ...
    try:
        ...
        if success_tts:
            print(f"[TASK] (4/4) 音声合成 完了。クライアントに通知します。")
            # ★ 処理完了をクライアントに通知
            ...
            print(f"[WARN] (4/4) 音声合成に失敗しました。")
            await websocket.send_json({"status": "error", "message": "音声合成に失敗しました。"})
        
        print(f"[TASK END] 全処理完了: {original_filename}")

    except Exception as e:
        print(f"[TASK ERROR] '{original_filename}' の処理中にエラーが発生しました: {e}")
        try:
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            print(f"[WS] ffmpeg 変換実行: {temp_input_path} -> {output_wav_path}")
            # 非同期でサブプロセスを実行
            proc = await asyncio.create_subprocess_exec(*cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            stdout, stderr = await proc.communicate()
            
            if proc.returncode != 0:
                print(f"[WS ERROR] ffmpeg 変換失敗: {stderr.decode()}")
                await websocket.send_json({"status": "error", "message": "音声形式の変換に失敗しました。"})
                continue
            
            print(f"[WS] ffmpeg 変換成功。")
        ...
        print("[WS] クライアントが切断しました。")
    except Exception as e:
        print(f"[WS ERROR] WebSocketエラー: {e}")
    finally:
