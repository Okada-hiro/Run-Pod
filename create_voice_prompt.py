...
def create_prompt():
    ...
    try:
        ...
        generated_npy = REFERENCE_AUDIO_NAME + ".npy"
        target_npy_path = PROMPT_TOKEN_FILE_NAME 

        if os.path.exists(generated_npy):
            print(f"[INFO] 生成された {generated_npy} を {target_npy_path} にリネームします。")
            shutil.move(generated_npy, target_npy_path)
            print(f"[SUCCESS] {target_npy_path} の準備ができました。")
        else:
            print(f"[ERROR] 期待された出力ファイル {generated_npy} が見つかりません。")
